...
public class GuardiansAvatar : MonoBehaviour
{
    public Camera avatarCamera;
    
    private readonly KeyCode[] keysOfInterest = new KeyCode[]    
    {
        KeyCode.W, KeyCode.A, KeyCode.S, KeyCode.D, KeyCode.M, KeyCode.Space, 
    ...
    void Shoot()
    {
        //CroquetBridge.Instance.SendToCroquet();
        CroquetBridge.Instance.SendToCroquetSync("event", "shoot");
    ...
    void Drive()
    {
        ...
        if (auto) {
            ...
            speed = gas * 20f * factor;
            steer = turn;
        }

        yawDelta = steer * dt * 100f;
        if (steer == 0 && speed == 0 && yawDelta == 0) return;
        ...
        Vector3 t = new Vector3(0, 0, speed) * (15f * dt);
        Vector3 tt = yawQ * t;
