...
private void JoystickExecuted(FingersJoystickScript script, Vector2 amount)
{
    ...
    float angle = Mathf.Atan2(amount.y, amount.x) * Mathf.Rad2Deg;

    // Determine the direction
    string direction = "";
    if (amount.y > 0) direction += "W";
    else if (amount.y < 0) direction += "S";

    if (amount.x > 0) direction += "D";
    else if (amount.x < 0) direction += "A";
    // Debug.Log(direction);

    // Calculate the speed of sending messages based on the magnitude of the vector
    float magnitude = amount.magnitude;
    float sendInterval = Mathf.Lerp(0.1f, 0.5f, 1.0f - magnitude); // Faster when closer to 1, slower when closer to 0

    // Send the direction to Croquet
    if (!string.IsNullOrEmpty(direction))
    {
        StartCoroutine(SendMovementCoroutine(direction, sendInterval));
    }

    // Move the object as before
    ...
    // Set custom input manager axis values
    CustomInputManager.Instance.SetAxis("Horizontal", amount.x);
    CustomInputManager.Instance.SetAxis("Vertical", amount.y);
}

        private IEnumerator SendMovementCoroutine(string direction, float interval)
        {
            // while (true)
            // {
                foreach (char dir in direction)
                {
                    // Debug.Log("Sending movement: " + dir);
                    // CroquetBridge.Instance.SendToCroquet("event", "keyDown", dir.ToString());
                }
